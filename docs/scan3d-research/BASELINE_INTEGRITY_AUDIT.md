...
7. `services/reconstruction_service.py::process_3d_scan_reconstruction` reads the supplied video through measured LIDRA, decodes selected frames, computes SIFT features and ratio-filtered matches, estimates the essential matrix, calls recoverPose, and triangulates points with positive-depth, reprojection and parallax checks. It chooses the best TWO views, creates image-plane Delaunay triangles with edge/depth limits, and writes OBJ, point-cloud PLY, STL and preview.
8. Node verifies the returned input identity and every output hash/size, writes immutable attempt provenance, and the worker publishes only under its held lease. X-Core discovers the registered assets and fetches them through the authenticated scan asset route.

This is image-derived sparse two-view computation. It is not dense full-arch reconstruction, calibrated geometry or clinical evidence. The adapter still performs acquisition before the reconstruction service repeats acquisition. The manifest now records the frames/settings actually used by reconstruction; eliminating duplicate acquisition is a separate optimization.
