...
const FeaturedDoctors = ({ appointments = [], onDoctorPress, onJoinCall }) => {
  ...
  const featuredDoctors = appointments.length > 0
    ? appointments.map(a => ({ id:a.id, name:a.dentist?.name || a.dentistName || 'Dr. Unknown', specialty:a.dentist?.specialty || a.specialty || 'Dental Specialist', experience:a.dentist?.experience || '5 yrs', rating:a.dentist?.rating || 4.8, reviews:a.dentist?.reviews || 100, price:a.price || a.dentist?.price || 0, status:getAppointmentStatus(a), nextSlot:formatAppointmentTime(a.startsAt), image:a.dentist?.image || a.dentist?.profilePicture || 'https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?w=400&h=400&fit=crop', verified:a.dentist?.verified || true, appointmentId:a.id, startsAt:a.startsAt, clinicName:a.clinic?.name || a.clinicName }))
    : [
        { id:'default-1', name:'Dr. Kriss Hemsworth', specialty:'Periodontic Specialist', experience:'6 yrs', rating:4.9, reviews:234, price:90, status:'Available Now', nextSlot:'2:30 PM', image:'https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?w=400&h=400&fit=crop', verified:true },
        { id:'default-2', name:'Dr. Amanda Hemsworth', specialty:'Orthodontic Specialist', experience:'8 years', rating:4.9, reviews:234, price:120, status:'Available Now', nextSlot:'2:30 PM', image:'https://images.unsplash.com/photo-1594824846003-5cee7a0e0f85?w=400&h=400&fit=crop', verified:true },
        { id:'default-3', name:'Dr. Sarah Mitchell', specialty:'Pediatric Dentist', experience:'5 years', rating:4.8, reviews:189, price:100, status:'Available Today', nextSlot:'4:15 PM', image:'https://images.unsplash.com/photo-1559839734-2b71ea197ec2?w=400&h=400&fit=crop', verified:true },
      ];

  function getAppointmentStatus(a){ if(!a.startsAt) return 'Scheduled'; const now=new Date(); const t=new Date(a.startsAt); const diffM=Math.floor((t-now)/60000); if(diffM<0) return 'Completed'; if(diffM<=30) return `Starts in ${diffM} min`; if(diffM<=60) return 'Starts in 1 hour'; const h=Math.floor(diffM/60); if(h<24) return `Starts in ${h} hours`; const d=Math.floor(h/24); return `In ${d} day${d>1?'s':''}`; }
  function formatAppointmentTime(s){ if(!s) return 'TBD'; const d=new Date(s); const h=d.getHours(); const m=d.getMinutes().toString().padStart(2,'0'); const ampm=h>=12?'PM':'AM'; const hh=h%12||12; return `${hh}:${m} ${ampm}`; }
                ...
                <LinearGradient colors={theme.gradients?.primary || ['#62109F','#982BEA']} style={{ borderRadius:24, padding:20, overflow:'hidden', shadowColor:'#667eea', shadowOffset:{ width:0, height:12 }, shadowOpacity:0.25, shadowRadius:20, elevation:12, minHeight:220 }} start={{ x:0, y:0 }} end={{ x:1, y:1 }}>
                  ...
                  <View style={{ flex:1, justifyContent:'space-between' }}>
                    <View style={{ flexDirection:'row', alignItems:'center', marginBottom:16 }}>
                      ...
                      <View style={{ flex:1, marginLeft:16 }}>
                        <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
                          ...
                          <Text style={{ fontSize:20, fontWeight:'bold', color:'white', textShadowColor:'rgba(0,0,0,0.1)', textShadowOffset:{ width:0, height:1 }, textShadowRadius:2 }}>${doctor.price}</Text>
                        </View>
                        ...
                        <View style={{ flexDirection:'row', alignItems:'center', backgroundColor:'rgba(255,255,255,0.2)', borderRadius:15, paddingHorizontal:10, paddingVertical:6, alignSelf:'flex-start', marginTop:4 }}>
                          ...
                          <View style={{ width:6, height:6, borderRadius:3, marginLeft:8, backgroundColor: doctor.status.includes('Starts') ? '#FF6B6B' : '#4ECDC4' }} />
                        </View>
                      </View>
                    </View>
                    <TouchableOpacity onPress={()=>_onJoin(doctor)} style={{ backgroundColor:'white', borderRadius:25, paddingVertical:12, flexDirection:'row', alignItems:'center', justifyContent:'center', shadowColor:'#000', shadowOffset:{ width:0, height:4 }, shadowOpacity:0.2, shadowRadius:8, elevation:5 }}>
                      ...
                      <Text style={{ fontWeight:'bold', marginLeft:8, fontSize:15, color:theme.colors.primary }}>{doctor.appointmentId ? 'Join Call' : 'Call Now'}</Text>
                    </TouchableOpacity>
                  </View>
                </LinearGradient>
              </TouchableOpacity>
