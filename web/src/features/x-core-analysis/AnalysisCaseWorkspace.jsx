...
import { caseItemLabel, RADIOGRAPH_TYPES, resolveSeriesUid, suggestRadiographType } from './domain.mjs';

const newId = () => globalThis.crypto?.randomUUID?.() || `case-item-${Date.now()}-${Math.random().toString(16).slice(2)}`;
          ...
          <div className="mx-auto max-w-6xl space-y-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={generate} disabled={saving} className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold hover:bg-emerald-500">Generate PDF semua citra</button>
              </div>
            </div>
                ...
                <article key={item.id} className="rounded-2xl border border-slate-800 bg-slate-900 p-4">
                  <div className="mb-3 flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      <span className={`mr-2 rounded-full px-2 py-1 text-[10px] ${item.render_storage_path ? 'bg-emerald-500/15 text-emerald-300' : 'bg-amber-500/15 text-amber-300'}`}>{item.render_storage_path ? 'snapshot siap' : 'belum capture'}</span>
                      ...
                      <button onClick={() => onOpenItem(activeCase, item, studies.find((study) => String(study.id) === String(item.study_id)))} className="rounded-lg bg-cyan-600 px-3 py-2 text-xs font-semibold">Buka viewer</button>
                  ...
                  <div className="grid gap-3 md:grid-cols-3">
                    ...
                    <label className="text-xs text-slate-400 md:col-span-3">Temuan per citra<textarea rows={2} className={`${inputClass} mt-1`} value={item.findings || ''} onChange={(e) => patchItem(item.id, { findings: e.target.value })} /></label>
                  </div>
                </article>
              ))}
            </section>
